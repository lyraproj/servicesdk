module github.com/puppetlabs/go-servicesdk

require (
	github.com/davecgh/go-spew v1.1.1 // indirect
	github.com/golang/protobuf v1.2.0
	github.com/hashicorp/go-plugin v0.0.0-20181030172320-54b6ff97d818
	github.com/pmezard/go-difflib v1.0.0 // indirect
	github.com/puppetlabs/data-protobuf v0.0.0-20180809162510-e2f18c438bd2
	github.com/puppetlabs/go-evaluator v0.0.0-20181112120037-0421d4ad8c2a
	github.com/puppetlabs/go-issues v0.0.0-20181112112915-c16bc8ecd50f
	github.com/puppetlabs/go-semver v0.0.0-20181112101422-34eb86658f1c
	github.com/stretchr/testify v1.2.2 // indirect
	golang.org/x/net v0.0.0-20181108082009-03003ca0c849
	google.golang.org/grpc v1.16.0
)
